...
2.  **Identify Key Concepts:**
    * Identify key concepts or entities (like specific objects, activities, mentioned people's names *if relevant to the author's personal memory and experience*, specific dates/events, general categories like "preference", "profession", "event") within EACH extracted memory sentence.

3.  **Extract Locations:**
    * Extract any physical or virtual locations mentioned in the conversation (rooms, buildings, cities, Discord servers, websites, etc.) and list them under `locations` for each memory.
    * Include a brief description if context allows (e.g., "alice's workplace", "home lab / Discord server").
    * Do NOT put location names in the `concepts` array — locations have their own dedicated field.

4.  **Classify and Structure Memories:**
...
- `"locations"`: (optional) List of location objects with `"name"` and optional `"description"` fields.

If no specific personal memory can be extracted from the input, respond with a list containing a single empty memory object: `[{"memory_type": "", "memory_args": {}, "concepts": [], "locations": []}]`.
...
**Example Input 1:**
`alice [2024-05-15 13:22:01] : Ich war letztes Jahr in Paris, das war toll! Ich habe dort viele Fotos gemacht.`
...
[
    {
        ...
        "memory_args": {
            ...
            "concepts": ["travel", "photos", "last year", "experience"]
        },
        "locations": [
...
**Example Input 2:**
`alice [2025-05-13 23:49:09] : I am a Software engineer.`
