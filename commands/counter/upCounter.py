...
class Command:
    ...
    def register_commands(self):
        @self.tree.command(name="upcounter", description="Increment a counter by name")
        async def upcounter(interaction: discord.Interaction, name: str):
                ...
                if user_data and user_data.get('counters'):
                    ...
                    for counter in counters:
                        if counter['name'] == name:
                            ...
                            await interaction.followup.send(f"Counter {name} incremented to {counter['count']}")
                            return
