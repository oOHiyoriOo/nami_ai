...
class Command:
    ...
    def register_commands(self):
        @self.tree.command(name="lastup", description="Increment the last added counter")
        async def lastup(interaction: discord.Interaction):
            ...
            try:
                ...
                if user_data and user_data.get('counters'):
                    ...
                    await interaction.followup.send(f"Counter {counters[-1]['name']} incremented to {counters[-1]['count']}")
                else:
                    await interaction.followup.send("No counters found to increment.")
