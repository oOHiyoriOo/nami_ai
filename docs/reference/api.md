...
| `stream` | boolean | No | Enable streaming response (**not yet supported** — returns HTTP 501) |
...
> **⚠️ Streaming is not yet supported.** Setting `stream: true` returns HTTP 501:
>
> ```json
> {
...
# Streaming (not yet supported — returns HTTP 501)
curl http://localhost:11434/api/chat -d '{
  "model": "llama2",
  "messages": [
...
| `stream` | boolean | No | Enable streaming (**not yet supported** — returns HTTP 501) |
...
```json
{
  ...
  "available_providers": ["ollama", "copilot"],
...
Get memory system diagnostics and statistics.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `user_id` | string | No | Filter analytics to a specific user. Omit for global stats. |
...
```json
{
  ...
  "access_patterns": {
    "most_accessed": ["user_preferences", "project_context"],
    "least_accessed": ["weather_cache", "temp_hold"]
  },
...
```bash
# Global analytics
curl http://localhost:11434/api/memory/analytics

# User-specific analytics
curl "http://localhost:11434/api/memory/analytics?user_id=alice"
```
...
List available models in OpenAI-compatible format. This is separate from `/api/tags` which returns the Ollama format.
