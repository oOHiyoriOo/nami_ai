...
class DynamicLoader:
    ...
    async def load_all(self, filter_fn: Optional[Callable[[Any], bool]] = None) -> List[Any]:
        """
        ...
        items = []
        python_files = [f for f in self.directory.iterdir() if f.suffix == '.py' and f.name != '__init__.py']
    ...
    async def _load_module(self, file_path: Path) -> Optional[Any]:
        """
...
class ToolLoader(DynamicLoader):
    ...
    async def load_tools(self) -> List[dict]:
        ...
        return [self._process_tool(tool_fn()) for tool_fn in raw_tools if callable(tool_fn)]
